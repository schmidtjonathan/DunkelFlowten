...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        if emb_off:
            ...
            self.emb_t_layers = nn.Sequential(
                nn.SiLU(),
                nn.Conv1d(
                    emb_channels,
                    2 * self.out_channels
    ...
    def _forward(self, x, emb, emb_t):
        ...
        emb_t_out = self.emb_t_layers(emb_t).type(h.dtype)
        while len(emb_out.shape) < len(h.shape):
            emb_out = emb_out[..., None]
            emb_t_out = emb_t_out[..., None]
        assert emb_out.shape == emb_t_out.shape
        if self.use_scale_shift_norm:
            out_norm, out_rest = self.out_layers[0], self.out_layers[1:]
            scale_, shift_ = torch.chunk(emb_out, 2, dim=1)
            scale_t, shift_t = torch.chunk(emb_t_out, 2, dim=1)
            scale = scale_ + scale_t
            shift = shift_ + shift_t
            h = out_norm(h) * (1 + scale) + shift
            h = out_rest(h)
        else:
            h = h + emb_out + emb_t_out
            h = self.out_layers(h)
...
class UNetModel(nn.Module):
    ...
    def __post_init__(self):
        ...
        self.hour_emb = nn.Embedding(4, self.time_embed_dim)
    ...
    def forward(self, x, timesteps, extra={}):
        ...
        y_month = extra["month"]
        y_hour = extra["hour"]
        # assert y_month.shape == x.shape[:2], (
        #     f"Month cond. have shape {y_month.shape}, which does not match the batch dimension of the input {x.shape}"
        # )
        emb_month = self.month_emb(y_month)
        # Hour embedding
        # assert y_hour.shape == x.shape[:2], (
        #     f"Hour cond. have shape {y_hour.shape}, which does not match the batch dimension of the input {x.shape}"
        # )
        emb_hour = self.hour_emb(y_hour)

        emb_month = torch.permute(emb_month, (0, 2, 1))
        emb_hour = torch.permute(emb_hour, (0, 2, 1))
        emb_t = emb_month + emb_hour
        ...
        for module in self.input_blocks:
            h = module(h, emb, emb_t)
            hs.append(h)
        h = self.middle_block(h, emb, emb_t)
