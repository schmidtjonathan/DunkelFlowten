...
from flow.utils import ModelWrapper
...
class BatchedFlow(torch.autograd.Function):
    ...
    def forward(flow, x, t, markov_order, batch_size, month_cond, hour_cond):
        with torch.no_grad():
            ...
            assert len(month_cond_batches) == len(hour_cond_batches) == num_batches
    ...
    def backward(ctx, grad_output):
        ...
        if batch_size is None:
            ...
            for batch_idx in range(num_batches):
                ...
                grad_splits.append(grad_output[current_idx : current_idx + split_size])
            ...
            for batch_idx in range(num_batches):
                ...
                # Move batch to GPU
                current_x = x_batches[batch_idx].to(model_device)
                current_grad = grad_splits[batch_idx].to(model_device)
                ...
                if is_first or is_last:
                    ...
                    if is_first:
                        grad_window[0, :markov_order] = current_grad[:markov_order]
                        grad_window[:, markov_order] = current_grad[markov_order:]
                    elif is_last:
                        grad_window[:, markov_order] = current_grad[:-markov_order]
                        grad_window[-1, -markov_order:] = current_grad[-markov_order:]
                else:
                    grad_window = torch.zeros_like(current_x.unflatten(1, (w, -1)))
                    ...
                    for window_pos in range(w):
                        ...
                        if target_idx < grad_x.size(0):
                            valid_batch_indices = torch.arange(
                                min(current_x.size(0), grad_x.size(0) - target_idx)
                            )
                            grad_x[
                                target_idx : target_idx + len(valid_batch_indices)
...
#     rnd_x = torch.randn(26, 5, 24, 40, dtype=torch.float32)
...
#             fold(
#                 flow_model(
#                     unfold(rnd_x.clone().to("cuda"), args_dict["markov_order"]),
#                     rnd_t.clone().to("cuda"),
#                 ),
#                 args_dict["markov_order"],
#                 mode="full",
#             )
#             .detach()
#             .cpu()
...
#     def DX2(x, t):
#         return fold(
#             flow_model(unfold(x, args_dict["markov_order"]), t),
#             args_dict["markov_order"],
#         ).mean()
