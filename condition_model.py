...
# ckp_path = "./checkpoints/model_1245097/checkpoint.pth"
# ckp_path = "./checkpoints/model_ckpt_preliminary/Felix/checkpoint.pth"
ckp_path = "./checkpoints/model_1303550/trained_model_1303550/checkpoint.pth"
...
print(f'Using normalization mode: {args_dict.get("data_norm_mode")}')
...
stats_dir = f'{data_path}/stats/'
dataset_test_gt = tds.ERADataset(
    data_path=f"{data_path}/eval/eval_fine.nc",
    cached=True,
    stats_dir=stats_dir,
...
dataset_test_obs = tds.ERADataset(
    data_path=f'{era5_dir}/training/{country_name}/{coarse_res}/eval/eval_coarse.nc',
    cached=True,
    stats_dir=stats_dir,
    norm_mode=args_dict.get("data_norm_mode"),
    order=args_dict["markov_order"],
...
savepath_samples = cur_out_dir / f"samples_{tr_str}_{fine_res}.nc"
fut.save_ds(ds=samples, filepath=savepath_samples)
savepath_gt = cur_out_dir / f"gt_{tr_str}_{fine_res}.nc"
fut.save_ds(ds=gt_ds, filepath=savepath_gt)
savepath_obs = cur_out_dir / f"obs_{tr_str}_{coarse_res}.nc"
fut.save_ds(ds=obs_ds, filepath=savepath_obs)
samples
# %%
# samples.to_netcdf(cur_out_dir / f"samples_{fine_res}.nc")
# gt_ds.to_netcdf(cur_out_dir / f"gt_{fine_res}.nc")
# obs_ds.to_netcdf(cur_out_dir / f"obs_{coarse_res}.nc")
